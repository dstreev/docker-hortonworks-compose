...
# Ambari Server
export AGENT=1
docker -H d6:2375 run -h ${CLUSTER_PREFIX}${AMBARI_INSTANCE}agent0${AGENT}.hdp.local --net-alias ${CLUSTER_PREFIX}${AMBARI_INSTANCE}agent0${AGENT}.hdp.local -P --name ${CLUSTER_PREFIX}${AMBARI_INSTANCE}agent0${AGENT} -d --network core --restart=unless-stopped -e "AMBARI_VERSION=${AMBARI_VERSION}" -e "CLUSTER_PREFIX=${CLUSTER_PREFIX}" -e "AMBARI_INSTANCE=${AMBARI_INSTANCE}" -e "AMBARI_SERVER=true" -e "AGENT=0${AGENT}" -e "REPO_BASE_URL=${REPO_BASE_URL}" -v "/var/local/hdp/${CLUSTER_PREFIX}${AMBARI_INSTANCE}/data:/data" dstreev/centos7_ambari

export AGENT=2
docker -H d7:2375 run -h ${CLUSTER_PREFIX}${AMBARI_INSTANCE}agent0${AGENT}.hdp.local --net-alias ${CLUSTER_PREFIX}${AMBARI_INSTANCE}agent0${AGENT}.hdp.local -P --name ${CLUSTER_PREFIX}${AMBARI_INSTANCE}agent0${AGENT} -d --network core --restart=unless-stopped -e "AMBARI_VERSION=${AMBARI_VERSION}" -e "CLUSTER_PREFIX=${CLUSTER_PREFIX}" -e "AMBARI_INSTANCE=${AMBARI_INSTANCE}" -e "AMBARI_AGENT=true" -e "AGENT=0${AGENT}" -e "REPO_BASE_URL=${REPO_BASE_URL}" -v "/var/local/hdp/${CLUSTER_PREFIX}${AMBARI_INSTANCE}/data:/data" dstreev/centos7_ambari

# Nifi
export AGENT=3
docker -H d3:2375 run -h ${CLUSTER_PREFIX}${AMBARI_INSTANCE}agent0${AGENT}.hdp.local --net-alias ${CLUSTER_PREFIX}${AMBARI_INSTANCE}agent0${AGENT}.hdp.local -P --name ${CLUSTER_PREFIX}${AMBARI_INSTANCE}agent0${AGENT} -d --network core --restart=unless-stopped -e "AMBARI_VERSION=${AMBARI_VERSION}" -e "CLUSTER_PREFIX=${CLUSTER_PREFIX}" -e "AMBARI_INSTANCE=${AMBARI_INSTANCE}" -e "AMBARI_AGENT=true" -e "NIFI_SERVER=true" -e "NIFI_VERSION=${NIFI_VERSION}" -e "AGENT=0${AGENT}" -e "REPO_BASE_URL=${REPO_BASE_URL}" -v "/var/local/hdp/${CLUSTER_PREFIX}${AMBARI_INSTANCE}/data:/data" dstreev/centos7_ambari
